...
class Bishop(Pieces):
    ...
    def __init__(self, colour, pos, hidden):
        ...
        self.cv2_coords = (200, 100*Pieces.colour_to_number[self.colour])
        self.__class__.bishop_instances[hidden].append(self)

    def get_legal_moves(self, game_board):
        legal_moves = []
        for i in range(4):
            for j in range(1, 8):
                pos = [self.queen_move_dict[2 * i + 1][0] * j + self.pos[0],
                       self.queen_move_dict[2 * i + 1][1] * j + self.pos[1]]
